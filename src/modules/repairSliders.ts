...
export const repairSliders = () => {
    ...
    let countSlideCurrent: Span | null = sliderWrapper!.querySelector('.slider-counter-content__current')
    let countSlideTotal: Span | null = sliderWrapper!.querySelector('.slider-counter-content__total')

    let actvieSlider: Div | null = document.querySelector('.types-repair.active-slider')

    let count = 0

    countSlideTotal!.textContent = `${actvieSlider!.querySelectorAll('div').length}`

    const changeListSlider = (targetBtn: Btn, classBtn: string, indexSlide: number, classSlider: string) => {
